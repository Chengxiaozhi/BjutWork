...
namespace Cool
{
    public partial class Task_execute03 : Form
    {
        ...
        private void Display()
        {
            ...
            con.Open();//开启连接
            string strcmd = "select TASK_ID,TASK_STATUS,gu.USER_REALNAME,cast(TASK_BIGTYPE as CHAR),TASK_APPLY_USERID,TASK_PLAN_BEGINTIME,TASK_PLAN_FINISHTIME FROM gunark_task_info gti LEFT JOIN gunark_user gu on gti.TASK_APPLY_USERID=gu.USER_POLICENUMB where (TASK_BIGTYPE='2'or TASK_BIGTYPE='4' or TASK_BIGTYPE='5' or TASK_BIGTYPE='6' or TASK_BIGTYPE='7' or TASK_BIGTYPE='9' or TASK_BIGTYPE='10') and (TASK_STATUS='3' or TASK_STATUS='10') and ('" + sss.ToString() + "' between TASK_PLAN_BEGINTIME and TASK_PLAN_FINISHTIME)";
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                else if (ds.Tables[0].Rows[i][3].Equals("10"))
                {
                    ds.Tables[0].Rows[i][3] = "枪支解封";
        ...
        private void Task_execute03_Load(object sender, EventArgs e)
        {
